...
class Net(torch.nn.Module):
    def __init__(self, config):
        ...
        self.num_rnn_sess_layers = config.num_rnn_sess_layers
        self.num_other_item_rnn_layers = config.num_other_item_rnn_layers
        ...
        # rnn for extracting session and user interest
        self.rnn_sess = torch.nn.LSTM(input_size = self.categorical_emb_dim *2, hidden_size = self.categorical_emb_dim//2, bidirectional=True , num_layers=self.num_rnn_sess_layers, batch_first=True)
        self.other_item_rnn = torch.nn.LSTM(input_size = self.categorical_emb_dim, hidden_size = self.categorical_emb_dim//2, bidirectional=True , num_layers=self.num_other_item_rnn_layers, batch_first=True)
    ...
    def forward(self, item_id, past_interactions, mask, price_rank, city, last_item, impression_index, cont_features, star, past_interactions_sess, past_actions_sess, last_click_item, last_click_impression, last_interact_index, neighbor_prices, other_item_ids, city_platform):
        ...
        # other items processed by rnn
        emb_other_item_ids_rnn, (hn_other_items, cn_other_items) = self.other_item_rnn(emb_other_item_ids)
        pooled_other_item_ids = F.max_pool1d(emb_other_item_ids_rnn.permute(0,2,1), kernel_size=emb_other_item_ids_rnn.size(1)).squeeze(2)
        ...
        emb_past_interactions_sess , (hn_past_interactions_sess, cn_past_interactions_sess) = self.rnn_sess(emb_past_interactions_sess)
